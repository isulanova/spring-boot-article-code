...
public class RoleService implements IRoleService {
  ...
  public List<RoleItemAdminResponse> getRoleList() {
    return roleRepo.findAll().stream()
        .map(item -> mapper.map(item, RoleItemAdminResponse.class))
        .toList();
  }
  ...
  public Optional<RolesByUserResponse> getRoleListByKeycloakId(final UUID keycloakId) {
    ...
    final List<RoleEntity> roleByKeycloakIdProj = roleRepo.findByKeycloakId(keycloakId);
  ...
  public Optional<RoleItemResponse> findByIdentifier(final UUID uuid) {
    return roleRepo.findById(uuid).map(item -> mapper.map(item, RoleItemResponse.class));
  }
  ...
  public boolean removeRole(final UUID id) {
    final Optional<RoleEntity> roleFromDb = roleRepo.findById(id);
