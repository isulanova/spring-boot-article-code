...
public class DbHealthIndicator extends AbstractHealthIndicator {
  ...
  protected void doHealthCheck(Health.Builder builder) {
    try {
      Optional<PgHealthChecker.DbStats> statsOptional = pgHealthChecker.getHealth();
      ...
      log.error("Error while collect application DB metrics." + "Message: {}", ex.getMessage());
      builder.status(new Status(HealthStatus.FATAL.name())).build();
    }
  }
}
